...
Шпаргалка по быстрой диагностике железа и настройке пользовательских скриптов в `~/bin` (пример — обёртка `docker_` для цветного вывода `docker ps` / `images`).

## Обзор железа и ресурсов

??? tip "Команды проверки RAM, диска, CPU, GPU"
    ```bash
    free -h
    df -hT
    lscpu
    ...
    # Видеокарта (общий случай)
    lspci | grep -i vga
    # затем детально по шине, например:
    # lspci -v -s 01:00.0
    sudo lshw -C display
...
??? tip "Свои скрипты как команды"
    ...
    Вставь в `~/bin/docker_` содержимое скрипта из блока ниже, сохрани и сделай исполняемым.

---

## Скрипт `docker_`

Поддерживаются: **`docker_ ps`**, **`docker_ ps -a`**, **`docker_ images`**, **`docker_ -h`**.
